...
import { saveCommissionerData } from '../../../utils/database';
...
const AddCommissioner = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await saveCommissionerData(currentUser.uid, formData);
  ...
  return (
    <div className="card shadow-sm p-4 border-0">
      <h3>Add Commissioner of Oaths</h3>
      <form onSubmit={handleSubmit} className="mt-4">
        ...
        <div className="mb-3">
          ...
          <input name="court" className="form-control" placeholder="e.g. High Court of The FCT Abuja" onChange={handleChange} required />
        </div>
        <div className="mb-4">
        ...
        <button type="submit" className="btn btn-primary" disabled={isSubmitting}>
          {isSubmitting ? 'Saving...' : 'Save Commissioner'}
        </button>
      </form>
    </div>
  );
